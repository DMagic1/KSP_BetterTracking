...
namespace BetterTracking.Unity
{
    ...
    public class ReorderableListElement : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler
    {
        ...
        private HeaderItem _parentHeader;
        ...
        public void OnBeginDrag(PointerEventData eventData)
        {
            ...
            _draggingObjectOriginalSize = new Vector2(262, 60);
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            worldPoint.x -= 250 * _scale;
            worldPoint.y += 20 * _scale;

            _draggingObject.position = worldPoint;

            if (_fakeElement.parent != _reorderableList.Content)
            ...
            for (int j = 0; j < _reorderableList.Content.childCount; j++)
            {
                ...
                {
                    float height = c.sizeDelta.y;
                    float cShift = c.position.y;

                    bool more = worldPoint.y > c.position.y - (height / 2) && height > 61;
                    ...
                    dist = Mathf.Abs(cShift - worldPoint.y);
                }
        ...
        private void RefreshSizes()
        {
            ...
            if (_fakeElementLE != null)
            {
                _fakeElementLE.preferredHeight = size.y;
                _fakeElementLE.preferredWidth = size.x;
                _fakeElementLE.minHeight = size.y;
                _fakeElementLE.minWidth = size.x;
            }
        }
        ...
        public void Init(ReorderableList reorderableList)
        {
            ...
            VesselGroup group = GetComponentInParent<VesselGroup>();

            if (group == null)
                return;

            _scale = group.Scale;
            _rect = group.GetComponent<RectTransform>();
            _parentHeader = group.Header;
        }
    }
}